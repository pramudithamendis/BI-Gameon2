...
--     (SELECT total_completed_amount FROM total_withdrawals_cumulative WHERE date_ < @yesterday ORDER BY date_ DESC LIMIT 1),
--     0
-- );

-- -- Get yesterday's new user count
-- SET @yesterday_count := COALESCE(
--     (SELECT total_completed_amount FROM total_withdrawals_daily WHERE date_ = @yesterday),
--     0
-- );

-- -- Insert or update the cumulative total for yesterday
-- INSERT INTO total_withdrawals_cumulative (date_, total_completed_amount)
...
SET @previous_total := COALESCE(
    (
        ...
        FROM total_withdrawals_cumulative
