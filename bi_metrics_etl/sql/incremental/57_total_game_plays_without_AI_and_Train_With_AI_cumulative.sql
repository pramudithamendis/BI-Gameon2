...
SET @previous_total := COALESCE(
    (SELECT total_sessions FROM total_game_plays_without_AI_and_Train_With_AI_cumulative WHERE date_ < @yesterday ORDER BY date_ DESC LIMIT 1),
    0
);
...
SET @yesterday_count := COALESCE(
    (SELECT total_sessions FROM total_game_plays_without_AI_and_Train_With_AI_daily WHERE session_date = @yesterday),
    0
);
