...
-- Get last week in Singapore timezone (ISO Week Mode)
SET @last_week_year := YEAR( CONVERT_TZ(DATE_SUB(NOW(), INTERVAL 1 WEEK), '+00:00', '+08:00') );
SET @last_week_number := WEEK( CONVERT_TZ(DATE_SUB(NOW(), INTERVAL 1 WEEK), '+00:00', '+08:00'), 1 );

INSERT INTO old_AI_matches_weekly (
    year_, week_number, total_ai_matches, total_player_wins, total_player_losses, total_spent_in_usd
)
SELECT 
