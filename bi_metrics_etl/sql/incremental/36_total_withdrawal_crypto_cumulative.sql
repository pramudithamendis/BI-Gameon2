...
-- Get cumulative total up to the day before yesterday
SET @previous_total := COALESCE(
    (SELECT total_completed_amount 
     FROM total_withdrawal_crypto_cumulative 
...
-- Get yesterday's totals from daily table
SET @yesterday_amount := COALESCE(
    (SELECT total_completed_amount 
     FROM total_withdrawal_crypto_daily 
     WHERE date_ = @yesterday),
    0
);
