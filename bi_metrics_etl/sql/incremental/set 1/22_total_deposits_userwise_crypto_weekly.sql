...
insert into total_deposits_crypto_weekly_userwise(user_id, email,first_name,last_name,year,week_number,week_label,total_completed_amount,total_transactions) 
...
JOIN gaming_app_backend.user u ON w.user = u.id 
	WHERE w.user_coin_transaction_method = 5 and w.created_at >= @cutoff   AND YEARWEEK(CONVERT_TZ(w.created_at, '+00:00', '+08:00'), 1) = @last_week_yearweek
