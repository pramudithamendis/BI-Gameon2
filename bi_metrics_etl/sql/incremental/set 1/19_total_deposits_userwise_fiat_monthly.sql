...
insert into total_deposits_fiat_monthly_userwise(user_id,email,first_name,last_name,month_,total_completed_amount,total_transactions) 
...
JOIN gaming_app_backend.user u ON w.user = u.id
	WHERE w.user_coin_transaction_method = 4 and w.created_at >= @cutoff AND DATE_FORMAT(CONVERT_TZ(w.created_at, '+00:00', '+08:00'), '%Y-%m') = @last_month
